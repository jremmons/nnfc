...
inline Tensor dispatch__cudnn_init_dropout_state(double dropout, bool train, int64_t dropout_seed, const Type & ty, int64_t device) {
  maybe_initialize_cuda(ty);
  AutoNoGIL no_gil;
  AutoGPU auto_gpu(device);
  return at::_cudnn_init_dropout_state(ty, dropout, train, dropout_seed);
}
...
inline Tensor dispatch_diagonal(const Tensor & self, int64_t offset) {

  AutoNoGIL no_gil;
  AutoGPU auto_gpu(self);
  return at::diagonal(self, offset);
}
...
inline Tensor & dispatch_sum(const Tensor & self, int64_t dim, ScalarType dtype, Tensor result) {

  AutoNoGIL no_gil;
  AutoGPU auto_gpu(result);
  return at::sum_out(result, self, dim, dtype);
}
inline Tensor dispatch_sum(const Tensor & self, int64_t dim, ScalarType dtype) {

  AutoNoGIL no_gil;
  AutoGPU auto_gpu(self);
  return self.sum(dim, dtype);
}
inline Tensor & dispatch_sum(const Tensor & self, int64_t dim, bool keepdim, ScalarType dtype, Tensor result) {

  AutoNoGIL no_gil;
  AutoGPU auto_gpu(result);
  return at::sum_out(result, self, dim, keepdim, dtype);
}
inline Tensor dispatch_sum(const Tensor & self, int64_t dim, bool keepdim, ScalarType dtype) {
