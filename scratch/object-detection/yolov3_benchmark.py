...
class YoloV3(nn.Module):
    ...
    def execute(self, x, end_layer):

        layers = 0
        end = False

        def layer_helper(func, layer_list, inp, layers, upsample=False):
            ...
            if len(layer_list) + layers >= end_layer:
                l = layer_list[:end_layer-layers]
                end = True
            else:
                l = layer_list
            ...
            return out, layers, end

        
        layer0, layers, end = layer_helper(self.apply_layers, self.layers[0], x, layers)
        if end:
            return [layer0]

        layer1, layers, end = layer_helper(self.apply_layers, self.layers[1], layer0, layers)
        if end:
            return [layer0, layer1]
       
        layer2, layers, end = layer_helper(self.apply_layers, self.layers[2], layer1, layers)
        if end:
            return [layer0, layer1, layer2]

        # YoloV3
        # process the intermediates for detections
        layer3, layers, end = layer_helper(self.apply_layers, self.layers[3], layer2, layers)
        if end:
            return [layer0, layer1, layer3]

        predict0, layers, end = layer_helper(self.apply_layers, self.layers[4] + self.layers[5], layer3, layers)
        if end:
            return [layer0, layer1, layer3, predict0]

        layer6, layers, end = layer_helper(self.apply_layers, self.layers[6], layer3, layers, upsample=True)
        if end:
            return [layer0, layer1, layer6]
        layer86 = torch.cat((layer6, layer1), 1)

        layer7, layers, end = layer_helper(self.apply_layers, self.layers[7], layer86, layers)
        if end:
            return [layer0, layer7]

        predict1, layers, end = layer_helper(self.apply_layers, self.layers[8] + self.layers[9], layer7, layers)
        if end:
            return [layer0, layer7, predict1]

        layer10, layers, end = layer_helper(self.apply_layers, self.layers[10], layer7, layers, upsample=True)
        if end:
            return [layer0, layer10]
        layer12 = torch.cat((layer10, layer0), 1)

        predict2, layers, end = layer_helper(self.apply_layers, self.layers[11] + self.layers[12], layer12, layers)
        if end:
            return [layer12, predict2]
...
def main():
    ...
    for img_path in args.image:
        ...
        image_original = image_original.resize((416, 416))
        ...
        for layer in range(1,54):
            times = []
            the_size = None
            the_shapes = ''
            for _ in range(16):
                t1 = timeit.default_timer()
                y = yolov3.execute(x, layer)
                size = 0
                shape = ''
                for l in y:
                    size += 4 * l.shape[1] * l.shape[2] * l.shape[3]
                    shape += str(l.shape)
                if size == 0:
                    size = 2
            ...
            #print('median', np.median(np.asarray(times)))
            print('layer', layer, 'median,', np.median(np.asarray(times)), ', mean,', np.mean(np.asarray(times)), ', size,', the_size, ', shape,', the_shape)
            
        # y = YoloV3.get_detections(y)
