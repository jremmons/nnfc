...
namespace codec {
    ...
    public:
        ...
        inline uint32_t symbol_denominator () const {
            return denominator;
        }
    ...
    public:
        ...
        void encode_symbol (const uint32_t symbol) {
            if (finished_) {
                throw std::runtime_error("`finished` already called, cannot encode more symbols.");
            }
            ...
            assert((range >= 1) or (sym_high < (std::numeric_limits<uint64_t>::max() / range)));
            assert((range >= 1) or (sym_low < (std::numeric_limits<uint64_t>::max() / range)));

            assert((denominator >= 1) and (low_ < (std::numeric_limits<uint64_t>::max() - (range * sym_high) / denominator) + 1));
            assert((denominator >= 1) and (low_ < (std::numeric_limits<uint64_t>::max() - (range * sym_low) / denominator)));
    ...
    public:
            ...
            
        {
            for(size_t i = 0; i < num_working_bits_ and i < data_.size(); i++) {
                value_ |= (static_cast<uint64_t>(data_.get_bit(i)) << (num_working_bits_ - i - 1));
                bit_idx_++;
            }
            assert(value_ <= max_);
        }
        ...
        uint32_t decode_symbol () {
            ...
            for(uint64_t sym_idx = 0; sym_idx < model_.size(); sym_idx++) {
                const std::pair<uint64_t, uint64_t> sym_prob = model_.symbol_numerator(sym_idx);
                const uint32_t denominator = model_.symbol_denominator();            
                assert(sym_prob.second > sym_prob.first);

                // check if overflow would happen
                assert((range >= 1) or (sym_prob.second < (std::numeric_limits<uint64_t>::max() / range)));
                assert((range >= 1) or (sym_prob.first < (std::numeric_limits<uint64_t>::max() / range)));

                assert((denominator >= 1) and (low_ < (std::numeric_limits<uint64_t>::max() - (range * sym_prob.second) / denominator) + 1));
                assert((denominator >= 1) and (low_ < (std::numeric_limits<uint64_t>::max() - (range * sym_prob.first) / denominator)));
