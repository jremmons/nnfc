...
codec::RGBp_to_YUV420p::RGBp_to_YUV420p(size_t height, size_t width)
    : context_(sws_getContext(width, height, AV_PIX_FMT_GBRP, width, height,
...
std::vector<uint8_t> codec::RGBp_to_YUV420p::convert(
  ...
  sws_scale(context_.get(), src_data, src_linesize, 0, height_, dest_data,
            dest_linesize);
...
codec::YUV420p_to_RGBp::YUV420p_to_RGBp(size_t height, size_t width)
    : context_(sws_getContext(width, height, AV_PIX_FMT_YUV420P, width, height,
...
std::vector<uint8_t> codec::YUV420p_to_RGBp::convert(
    std::vector<uint8_t>& src_frame) {
...
codec::RGB24_to_YUV422p::RGB24_to_YUV422p(size_t height, size_t width)
    : context_(sws_getContext(width, height, AV_PIX_FMT_RGB24, width, height,
                              AV_PIX_FMT_YUV422P, 0, nullptr, nullptr, nullptr),
               [](SwsContext* ptr) { sws_freeContext(ptr); }),
      height_(height),
      width_(width) {
  av_log_set_level(AV_LOG_QUIET);
...
std::vector<uint8_t> codec::RGB24_to_YUV422p::convert(
    std::vector<uint8_t>& src_frame) {
  assert(src_frame.size() == static_cast<size_t>(3 * height_ * width_));
...
codec::YUV422p_to_RGB24::YUV422p_to_RGB24(size_t height, size_t width)
    : context_(sws_getContext(width, height, AV_PIX_FMT_YUV422P, width, height,
...
std::vector<uint8_t> codec::YUV422p_to_RGB24::convert(
    std::vector<uint8_t>& src_frame) {
