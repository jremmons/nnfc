...
nnfc::H265ImageEncoder::H265ImageEncoder(int quality) :
    encoder_(quality)
{}

vector<uint8_t> nnfc::H265ImageEncoder::forward(nn::Tensor<float, 3> input) {
  const uint64_t dim0 = input.dimension(0);
  const uint64_t dim1 = input.dimension(1);
  const uint64_t dim2 = input.dimension(2);

  if (dim0 != 3) {
    throw std::runtime_error("The H265ImageEncoder expects a 3-channel input");
  }
...
nn::Tensor<float, 3> nnfc::H265ImageEncoder::backward(
    nn::Tensor<float, 3> input) {
  return input;
}

nnfc::H265ImageDecoder::H265ImageDecoder() :
    decoder_()
{}

nnfc::H265ImageDecoder::~H265ImageDecoder() {}

nn::Tensor<float, 3> nnfc::H265ImageDecoder::forward(vector<uint8_t> input) {
  uint64_t dim0;
  uint64_t dim1;
  uint64_t dim2;
...
nn::Tensor<float, 3> nnfc::H265ImageDecoder::backward(
    nn::Tensor<float, 3> input) {
  return input;
}
